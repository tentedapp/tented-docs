...
Contacts created by the sync show **HubSpot** as their source. Each sync applies only the properties HubSpot changed, so an edit made in Tented survives an unrelated HubSpot change; if the same field changed on both sides between two syncs, the HubSpot value wins. With two-way sync, a Tented value that is not one of a HubSpot dropdown's options (for example a lifecycle stage HubSpot does not have) is left out, and the rest of the contact is still sent. Changing the mapping or the list later re-reads the account with the new settings; contacts already in Tented are updated, not duplicated.
