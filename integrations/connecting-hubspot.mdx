...
Contacts created by the sync show **HubSpot** as their source. If the same field changed on both sides between two syncs, the HubSpot value wins. Changing the mapping or the list later re-reads the account with the new settings; contacts already in Tented are updated, not duplicated.
