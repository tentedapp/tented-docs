...
- **Capitalization and separators don't matter — the keyword does.** `:titlecase`, `:Title Case`, and `:title_case` all resolve to title case, but shorthands and typos (`:title`, `:upper`, `:titlcase`) are flagged as invalid rather than guessed at.
