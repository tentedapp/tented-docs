...
<Note>
  Sending domains are available on **every plan, Free included** — verifying a domain is part of getting set up, not a paid feature. Free-plan limits apply to sending volume instead: 100 emails per day, and [flows require Pro or Max to activate](/email/triggered-flows#activate-it).
</Note>
...
- **Operational sends bypass the unsubscribe preference only** (password resets, security notices) — invalid, hard-bounced addresses stay excluded even there. Keep that toggle off for anything promotional.
