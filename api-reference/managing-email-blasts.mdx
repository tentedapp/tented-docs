...
<Info>
  The attached email must be **approved** before the blast can be scheduled or sent. Inline emails start as drafts — take the `email.email_id` from the response and approve it via [`POST /v1/emails/{emailId}/approve`](/api-reference/approving-emails#approve-and-unapprove).
</Info>
...
```json
{
  "error": "Sending this blast to 250 recipients would exceed the free plan's daily limit of 100 emails (100 remaining today). Upgrade to Pro or Max to send more.",
  "error_code": "free_plan_email_limit",
  "details": {
...
| `403 Forbidden` | Send Now over a Free-plan limit (`free_plan_email_limit`, with the limit details) — the blast stays a `draft` |
