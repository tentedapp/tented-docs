...
description: "Create tents programmatically from prompts or approved templates, attach files, and poll generation status through Tented's public API."
---

## Overview

Tented's public API lets you create new tents from your own systems without using the app UI. The current public surface is focused on the initial generation workflow:

- Create a tent from a prompt
- Create a tent from an approved template
...
<Info>
  The public API is asynchronous. `POST /v1/tents` returns immediately with a pending job, and you then poll `GET /v1/tents/{tentId}` until the status becomes `completed` or `failed`.
</Info>
...
If you already have an approved template in the same workspace, you can start from that template instead:

```json
{
  "name": "Spring campaign landing page",
  "prompt": "Adapt it for our spring release with updated pricing, customer logos, and a contact form",
