...
Proposed mappings never send two columns to the same field. When two columns match one field, the column matched by its header name wins over an AI suggestion, then the earlier column wins. The other column is left unmapped with a `reason` naming the column that claimed the field, so it shows up in `field_creation_candidates`. For example, in a Shopify customer export, `Phone` maps to `phone` and `Default Address Phone` is left for a custom field.
...
| Field | Type | Required | Notes |
| --- | --- | --- | --- |
| `mappings` | `array` | Yes | `{header, target_field, target_kind, overwrite_behavior?}` per column. Each field can be the target of only one column; mapping two columns to the same field returns `400` |
