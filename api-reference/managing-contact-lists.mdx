...
`rule_issues` lists custom fields a dynamic list's rules still name after the integration that owned them was disconnected (each entry has `label`, `provider` and `api_name`). Those conditions match nobody until the rules are changed; it is `[]` for every other list.
