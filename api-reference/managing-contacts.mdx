...
description: "Look up, create, update, and delete contacts through the public API in batches of up to 25 records."
---
...
Use these endpoints to sync CRM contacts from external systems. Each write request accepts an `items` array with up to `25` records.
...
Find a single contact by identifier. At least one of `email` or `phone` is required — this endpoint is a lookup, not a full listing.
