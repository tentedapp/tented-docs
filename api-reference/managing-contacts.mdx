...
Returns `{items, pagination}`. Each item carries every standard contact field (address fields, `unsubscribed`, `marketing_email_subscribed`, `marketing_sms_subscribed`, and so on), the same as [Get a Contact](#get-a-contact). List items do not include `custom_fields` — fetch a single contact for those.
...
Returns `{items, pagination}`. Like [List Contacts](#list-contacts), each item carries every standard contact field but not `custom_fields`.
